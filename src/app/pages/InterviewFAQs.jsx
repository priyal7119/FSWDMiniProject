...
import {
  ...
  Award, ArrowRight, Bookmark, Archive, CheckCircle
} from "lucide-react";
import { getFAQs, incrementPrepCount, addBookmark } from "../utils/api.js";
...
export function InterviewFAQs() {
  ...
  const handleSave = async (faq) => {
    try {
      ...
      success(`Question synchronized to Archive Vault.`);
    } catch (err) {
        ...
        <div className="flex flex-wrap gap-4 mb-16 overflow-x-auto pb-4 no-scrollbar">
          {faqRoles.map((role) => (
            <button
              ...
              className={`flex items-center gap-3 px-8 py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all whitespace-nowrap shadow-sm border ${activeRole === role.id
        ...
        <div className="space-y-6">
          {filteredFaqs.length > 0 ? (
            filteredFaqs.map((faq) => (
              <div
                key={faq.id}
                className={`bg-card border transition-all duration-500 rounded-[2.5rem] overflow-hidden ${expandedId === faq.id ? "border-primary shadow-2xl shadow-primary/5" : "border-border shadow-sm hover:border-muted-foreground/30"
                  }`}
              >
                <button
                  onClick={() => {
                    ...
                    <div className="pt-6 border-t border-border">
                      ...
                      <div className="mt-8 flex items-center justify-between gap-4">
                        <span className="text-xs font-bold text-muted-foreground flex items-center gap-2">
                          <CheckCircle size={14} className="text-teal-500" /> Expert Approved
                        </span>
                        <button
                          onClick={() => handleSave(faq)}
                          className="flex items-center gap-2 px-6 py-2.5 bg-primary/10 text-primary border border-primary/20 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-primary hover:text-white transition-all transition-all shadow-sm"
                        >
